...
export default function ProdutoUpdateForm(props) {
  const {
    id: idProp,
    produto: produtoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = produtoRecord
      ? { ...initialValues, ...produtoRecord }
      : initialValues;
    setNome(cleanValues.Nome);
    setDescricao(cleanValues.Descricao);
    setVrUnitario(cleanValues.VrUnitario);
    setUrlFoto(cleanValues.UrlFoto);
    setErrors({});
  };
  const [produtoRecord, setProdutoRecord] = React.useState(produtoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Produto, idProp)
        : produtoModelProp;
      setProdutoRecord(record);
    };
    queryData();
  }, [idProp, produtoModelProp]);
  React.useEffect(resetStateValues, [produtoRecord]);
  const validations = {
    Nome: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Produto.copyOf(produtoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ProdutoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nome"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || produtoModelProp)}
